...
from rpn.rpn import get_rpn_testbatch_optimal, get_rpn_trainbatch_optimal, get_rpn_testbatch, get_rpn_batch, assign_anchor
from rcnn import get_rcnn_testbatch, get_rcnn_batch

class TestLossLoader(mx.io.DataIter):
    def __init__(self, roidb, config, batch_size=1, shuffle=False,
                 has_rpn=False):
        super(TestLossLoader, self).__init__()
    ...
    def get_batch(self):
        ...
        if self.has_rpn:
            data, label, im_info = get_rpn_batch(roidb, self.cfg)
        else:
...
class TestBatchLoader(mx.io.DataIter):
    def __init__(self, roidb, name, config, batch_size=32, shuffle=False,
                 has_rpn=False):
        super(TestBatchLoader, self).__init__()

        # save parameters as properties
        if name != '':
            assert os.path.exists(name), '{} does not exist'.format(name)
            with open(name) as f:
                self.optimals = cPickle.load(f) # optimals is a dictionary (frame_id: resolution)
        self.cfg = config
        self.roidb = roidb
        self.batch_size = batch_size
        self.shuffle = shuffle
    ...
    def get_batch(self):
        ...
        if self.has_rpn:
            if self.cfg.OPTIMAL:
                data, label, im_info = get_rpn_testbatch_optimal(roidb, self.optimals, self.cfg)
            else:
                data, label, im_info = get_rpn_testbatch(roidb, self.cfg)
        else:
            data, label, im_info = get_rcnn_testbatch(roidb, self.cfg)
        self.data = [[mx.nd.array(np.squeeze([idata[name] for idata in data])) for name in self.data_name]]
        self.im_info = im_info

class TestLoader(mx.io.DataIter):
    def __init__(self, roidb, name, config, batch_size=1, shuffle=False,
                 has_rpn=False):
...
class AnchorLoader(mx.io.DataIter):
    ...
    def get_batch(self):
        ...
        for islice in slices:
            ...
            data, label, _ = get_rpn_batch(iroidb, self.cfg)
...
class TestScaleRegLoader(mx.io.DataIter):
    def __init__(self, roidb, name, config, batch_size=1, shuffle=False,
                 has_rpn=False):
        super(TestScaleRegLoader, self).__init__()
...
class TrainScaleRegLoader(mx.io.DataIter):
    def __init__(self, roidb, name, config, batch_size=1, shuffle=False,
                 ctx=None, work_load_list=None, has_rpn=False):
        super(TrainScaleRegLoader, self).__init__()
    ...
    def get_batch(self):
        ...
        for idx, islice in enumerate(slices):
            iroidb = [roidb[i] for i in range(islice.start, islice.stop)]
            if self.has_rpn:
                data, label, im_info = get_rpn_trainbatch_optimal(iroidb, self.optimals, self.cfg)
                rst.append({'data': data[0], 'label': label[0]})
        all_data = [_['data'] for _ in rst]
        all_label = [_['label'] for _ in rst]
        self.data = [[mx.nd.array(data[key]) for key in self.data_name] for data in all_data]
        self.label = [[mx.nd.array([label[key]]) for key in self.label_name] for label in all_label]