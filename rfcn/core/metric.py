...
class MSE(mx.metric.EvalMetric):
    def __init__(self):
        super(MSE, self).__init__('mse')
        self.pred, self.label = ['scale', 'lro'], ['scale_target']

    def update(self, labels, preds):
        pred = preds[self.pred.index('scale')].asnumpy()
        label = labels[self.label.index('scale_target')].asnumpy()

        self.sum_metric += np.mean((np.power(pred - label, 2)))
        self.num_inst += len(preds)
