...
config.OPTIMAL = False # Rudy: for optimal image scales
config.KAPPA = 0 # Rudy: for optimal image scales
config.MAX_REG_SCALE = 600
config.MIN_REG_SCALE = 240
...
config.TRAIN.rfcn_model_prefix = ''
config.TRAIN.regressor_model_prefix = ''
...
config.TEST.SOFTNMS_THRESH = 0.6
        ...
        for k, v in exp_config.items():
            if k in config:
                if isinstance(v, dict):
                    ...
                    if k == 'SCALES':
                        total_scales = len(v)/2
                        config[k] = []
                        for i in range(total_scales):
                            config[k].append(tuple(v[i*2:(i+1)*2]))
                    else:
                        config[k] = v
            else:
