...
from operator_py.proposal_target_nogt import *
...
class resnet_v1_101_rfcn(Symbol):
    ...
    # Rudy
    def get_testloss_symbol(self, cfg):
        ...
        # shared convolutional layers
        conv_feat = self.get_resnet_v1(data)
        # split the feature maps into two halves, one going to RPN, the other going to the detection module
        conv_feats = mx.sym.SliceChannel(conv_feat, axis=1, num_outputs=2)
        ...
        # if normalized, then inverse the normalization using the mean and std
        if cfg.network.NORMALIZE_RPN:
            rpn_bbox_pred = mx.sym.Custom(
                bbox_pred=rpn_bbox_pred, op_type='rpn_inv_normalize', num_anchors=num_anchors,
                bbox_mean=cfg.network.ANCHOR_MEANS, bbox_std=cfg.network.ANCHOR_STDS)
        ...
        if cfg.TEST.CXX_PROPOSAL:
            # mx.contrib.sym.MultiProposal: Generate region proposals via RPN
        ...
        rfcn_bbox = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=7*7*4*num_reg_classes, name="rfcn_bbox")
        # PSROIPooling:
        # Performs region-of-interest pooling on inputs. Resize bounding box coordinates by spatial_scale and crop input feature maps accordingly.
        # The cropped feature maps are pooled by max pooling to a fixed size output indicated by pooled_size.
        # batch_size will change to the number of region bounding boxes after PSROIPooling
        psroipooled_cls_rois = mx.contrib.sym.PSROIPooling(name='psroipooled_cls_rois', data=rfcn_cls, rois=rois, group_size=7, pooled_size=7,
                                                   output_dim=num_classes, spatial_scale=0.0625)
        psroipooled_loc_rois = mx.contrib.sym.PSROIPooling(name='psroipooled_loc_rois', data=rfcn_bbox, rois=rois, group_size=7, pooled_size=7,
                                                   output_dim=8, spatial_scale=0.0625)
        ...
        # classification
        cls_score = mx.sym.Reshape(name='cls_score_reshape', data=cls_score, shape=(-1, num_classes))
        cls_loss = mx.sym.SoftmaxActivation(name='cls_prob', data=cls_score) + 1e-14
        # cross entropy loss
        cls_loss = mx.sym.pick(cls_loss, label.astype('int32'), axis=1)
        cls_loss = -1 * mx.sym.log(name='cls_loss_', data=cls_loss)
        #cls_loss = mx.sym.sum(name='cls_loss', data=cls_loss)
        ...
        # bbox_target = mx.sym.Reshape(data=bbox_target, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes), name='bbox_tgt_reshape')

        # bounding box regression
        bbox_loss = bbox_weight * mx.sym.smooth_l1(name='bbox_loss_', data=(bbox_pred - bbox_target), scalar=1.0)
        #bbox_loss = mx.sym.sum(data=bbox_loss, name='bbox_loss')

        bbox_pred = mx.sym.Reshape(data=bbox_pred, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes), name='bbox_pred_reshape')

        cls_loss = mx.sym.identity(cls_loss, name='cls_loss')
        bbox_loss = mx.sym.identity(bbox_loss, name='bbox_loss')
        label = mx.sym.identity(label, name='label')
        rois = mx.sym.identity(rois, name='rois')
        # group output
        group = mx.sym.Group([rois, cls_prob, bbox_pred, cls_loss, bbox_loss, label])
        self.sym = group
        return group
    

    # Rudy, scale_regression layer
    def get_scalereg_symbol(self, cfg):
        ...
        data = mx.sym.Variable(name="data")
        scale_target = mx.sym.Variable(name="scale_target")
        ...
        # scale regression
        grow_filter = mx.sym.Convolution(data=rfcn_feat, kernel=(3, 3), num_filter=512, name="grow")
        shrink_filter = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=512, name="shrink")
        grow_relu = mx.symbol.Activation(name='grow_relu', data=grow_filter , act_type='relu')
        shrink_relu = mx.symbol.Activation(name='shrink_relu', data=shrink_filter , act_type='relu')
        grow_feat = mx.sym.Pooling(name='ave_grow', data=grow_relu, pool_type='avg', global_pool=True, kernel=(7, 7))
        shrink_feat = mx.sym.Pooling(name='ave_shrink', data=shrink_relu, pool_type='avg', global_pool=True, kernel=(7, 7))
        scale_reg_feat = mx.sym.concat(grow_feat, shrink_feat, dim=1)
        scale = mx.sym.FullyConnected(name='scale', data=scale_reg_feat, num_hidden=1)
        lro = mx.sym.LinearRegressionOutput(data=scale, label=scale_target, name="lro")

        group = mx.sym.Group([mx.sym.BlockGrad(scale), lro])
        self.sym = group
        return group

    # Rudy, scale_regression layer
    def get_scalereg_1x1_symbol(self, cfg):
        ...
        shrink_relu = mx.symbol.Activation(name='shrink_relu', data=shrink_filter , act_type='relu')
        shrink_feat = mx.sym.Pooling(name='ave_shrink', data=shrink_relu, pool_type='avg', global_pool=True, kernel=(7, 7))
        scale = mx.sym.FullyConnected(name='scale', data=shrink_feat, num_hidden=1)
        lro = mx.sym.LinearRegressionOutput(data=scale, label=scale_target, name="lro")
    ...
    def get_test_scalereg_symbol(self, cfg):
        # config alias for convenient
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        num_anchors = cfg.network.NUM_ANCHORS
    ...
    def get_test_adaptive_scale_1x1_symbol(self, cfg):
        ...
        # group output
        group = mx.sym.Group([rois, cls_prob, bbox_pred, scale])
        self.sym = group
        return group

    def get_test_adaptive_scale_symbol(self, cfg):
    ...
    def init_scale_reg(self, cfg, arg_params, aux_params):
        arg_params['grow_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['grow_weight'])
        arg_params['grow_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['grow_bias'])

        arg_params['shrink_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['shrink_weight'])
        arg_params['shrink_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['shrink_bias'])

        arg_params['scale_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['scale_weight'])
        arg_params['scale_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['scale_bias'])

    def init_scale_reg_1x1(self, cfg, arg_params, aux_params):
        arg_params['shrink_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['shrink_weight'])
        arg_params['shrink_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['shrink_bias'])
