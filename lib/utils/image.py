...
def get_optimal_image(roidb, optimals, config):
    """
    preprocess image and return processed roidb
    downsample the image to according optimal size (config.OPTIMAL)
    ...
    for i in range(num_images):
        roi_rec = roidb[i]
        if int(roi_rec['frame_id']) in optimals:
            optimal = optimals[int(roi_rec['frame_id'])] # (SCALES[0], SCALES[1])
        else:
            optimal = config.SCALES[0]
        assert os.path.exists(roi_rec['image']), '%s does not exist'.format(roi_rec['image'])
        im = cv2.imread(roi_rec['image'], cv2.IMREAD_COLOR|cv2.IMREAD_IGNORE_ORIENTATION)
        if roidb[i]['flipped']:
            im = im[:, ::-1, :]
        new_rec = roi_rec.copy()
        target_size = optimal[0]
        max_size = optimal[1]
...
def get_optimal_image_label(roidb, optimals, config):
    """
    preprocess image and return processed roidb
    downsample the image to according optimal size (config.OPTIMAL)
    :param roidb: a list of roidb
    ...
    for i in range(num_images):
        roi_rec = roidb[i]
        if roi_rec['frame_id'] in optimals:
            optimal = optimals[roi_rec['frame_id']] # (SCALES[0], SCALES[1])
        else:
            optimal = config.SCALES[0] # Default is picking the first scale (should be 600, 2000)
        assert os.path.exists(roi_rec['image']), '%s does not exist'.format(roi_rec['image'])
        im = cv2.imread(roi_rec['image'], cv2.IMREAD_COLOR|cv2.IMREAD_IGNORE_ORIENTATION)
        if roidb[i]['flipped']:
            im = im[:, ::-1, :]
        ...
        max_scale, min_scale = config.MAX_REG_SCALE, config.MIN_REG_SCALE
        x = (float(optimal[0]) / target_size)
        x = normalize(x, float(max_scale)/float(min_scale), float(min_scale)/float(max_scale))
        label = 2*x-1
    ...
    return processed_ims, processed_roidb, processed_labels


def get_scaled_frame(roidb, config, target_shortest):
    """
    preprocess image and return processed roidb
    :param roidb: the roidb that contains the target image
    :return: img in mxnet format
    roidb add new item['im_info']
    0 --- x (width, second dim of im)
    |
    y (height, first dim of im)
    ...
    im, im_scale = resize(im, target_shortest, max_size, stride=config.network.IMAGE_STRIDE)
    im_tensor = transform(im, config.network.PIXEL_MEANS)
    im_info = [im_tensor.shape[2], im_tensor.shape[3], im_scale]
    ...
    data = [{'data': im_tensor,
            'im_info': im_info}]
    data = [[mx.nd.array([np.squeeze(idata[name]) for idata in data]) for name in data_name]]
