...
from utils.image import get_image, get_optimal_image, get_optimal_image_label, get_pair_image, tensor_vstack
from generate_anchor import generate_anchors
from bbox.bbox_transform import bbox_overlaps, bbox_transform


def get_rpn_trainbatch_optimal(roidb, optimals, cfg):
    """
    return a dict of testbatch
    :param roidb: ['image', 'flipped']
    :return: data, label, im_info
    """
    imgs, roidb, labels = get_optimal_image_label(roidb, optimals, cfg)
    im_array = imgs
    ...
    label = [{'scale_target': labels[i]} for i in range(len(labels))]

    return data, label, im_info

def get_rpn_testbatch_optimal(roidb, optimals, cfg):
    """
    return a dict of testbatch
    :param roidb: ['image', 'flipped']
    :return: data, label, im_info
    """
    imgs, roidb = get_optimal_image(roidb, optimals, cfg)
    im_array = imgs
    im_info = [np.array([roidb[i]['im_info']], dtype=np.float32) for i in range(len(roidb))]
