...
class ImageNetVID(IMDB):
    ...
    def load_image_set_index(self):
        ...
        with open(image_set_index_file) as f:
            ...
            for line in f:
                l = line.strip().split(' ')
                if len(l) == 2:
                    self.image_set_index.append(l[0])
                    self.frame_id.append(l[1])
                    self.pattern.append(None)
                    self.frame_seg_id.append(0)
                    self.frame_seg_len.append(0)
                else:
                    self.image_set_index.append('%s/%06d' % (l[0], int(l[2])))
                    self.pattern.append(l[0]+'/%06d')
                    self.frame_id.append(int(l[1]))
                    self.frame_seg_id.append(int(l[2]))
                    self.frame_seg_len.append(int(l[3]))
    ...
    def image_path_from_index(self, index):
        ...
        elif self.det_vid == 'MIX':
            if os.path.isfile(os.path.join(self.data_path, 'Data', 'DET', index + '.JPEG')):
                image_file = os.path.join(self.data_path, 'Data', 'DET', index + '.JPEG')
            else:
    ...
    def load_vid_annotation(self, iindex):
        ...
        if self.pattern[iindex] is not None:
        ...
        if self.det_vid == 'DET':
            ...
            if os.path.isfile(os.path.join(self.data_path, 'Annotations', 'DET', index + '.xml')):
                filename = os.path.join(self.data_path, 'Annotations', 'DET', index + '.xml')
            else:
...
###################################################################################################
    ...
    def annotate_detections(self, optimal_output_path, detections, roidb):
        """
        draw detected results on image with ground truth
        :param detections: result matrix, [bbox, confidence]
        :return: None
        """
        print('Annotating images with ground truth and detection results...')

        with open(optimal_output_path) as f:
            optimals = cPickle.load(f)

        # make all these folders for results
        result_dir = os.path.join('/data', 'annotated_img')
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)

        print(len(self.image_set_index), len(optimals))
        average_size = {600: [], 480: [], 304: [], 128: []}
        for im_ind, index in enumerate(self.image_set_index):
            if im_ind >= len(optimals):
                break
            if optimals[roidb[im_ind]['frame_id']][0] != 600:
                im = cv2.imread(roidb[im_ind]['image'])
            # area = roidb[im_ind]['height'] * roidb[im_ind]['width']
                # ground truth box is in [x1, y1, x2, y2] format
                for gt_idx in range(roidb[im_ind]['boxes'].shape[0]):
                    box = roidb[im_ind]['boxes'][gt_idx]
                    label = roidb[im_ind]['gt_classes'][gt_idx]
                    bbox_area = (int(box[2])-int(box[0]))*(int(box[3])-int(box[1]))
                # average_size[optimals[roidb[im_ind]['frame_id']][0]].append((float(bbox_area)/area))
        # for key in average_size:
        #     print 'Size {}, Mean: {}, Std: {}'.format(key, np.mean(average_size[key]), np.std(average_size[key]))
                # Draw ground truth with green bounding boxes
                cv2.rectangle(im, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 255, 0), 1)
                cv2.putText(im, self.classes[label], (int(box[0]),int(box[1]+20)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                ...
                for cls_ind, cls in enumerate(self.classes):
                    ...
                    for k in range(dets.shape[0]):
                        if dets[k, -1] > 0.1: # Better than random guessing
                            box = dets[k, :4]
                            # Draw detection results with blue bounding boxes
                            cv2.rectangle(im, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), 1)
                            cv2.putText(im, '{} {:.2f}'.format(self.classes[cls_ind], dets[k, -1]), (int(box[0]),int(box[1]+20)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
                
                cv2.imwrite('{}/{}_{}.jpeg'.format(result_dir, str(roidb[im_ind]['frame_id']), str(optimals[roidb[im_ind]['frame_id']][0])), im)

    def evaluate_detections(self, detections, roidb):
        """
        ...
        self.write_vid_results(detections, roidb)
    ...
    def write_vid_results(self, all_boxes, roidb):
        """
                ...
                for cls_ind, cls in enumerate(self.classes):
                    ...
                    for k in range(dets.shape[0]):
                        f.write('{:d} {:d} {:.4f} {:.2f} {:.2f} {:.2f} {:.2f}\n'.
                                format(int(roidb[im_ind]['frame_id']), int(cls_ind), dets[k, -1],
                                       dets[k, 0], dets[k, 1], dets[k, 2], dets[k, 3]))
                                # format(int(self.frame_id[im_ind]), cls_ind, dets[k, -1],
                                #        dets[k, 0], dets[k, 1], dets[k, 2], dets[k, 3]))
    ...
    # Rudy
    def do_python_eval_filename(self, filename):
        """
        ...
        for threshold in np.arange(0.5, 1, 0.05):
            print('Calculating Threshold {}'.format(threshold))
            aps.append(vid_eval(filename, annopath, imageset_file, self.classes_map, annocache, ovthresh=threshold))
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('AP for {} = {:.4f}'.format(cls, aps[0][cls_ind-1]))
            info_str += 'AP for {} = {:.4f}\n'.format(cls, aps[0][cls_ind-1])
        print('Mean AP@0.5 = {:.4f}'.format(np.mean(aps[0])))
        print('Mean AP@[0.5:0.95] = {:.4f}'.format(np.mean(np.mean(aps, 1))))
        info_str += 'Mean AP@0.5 = {:.4f}\n\n'.format(np.mean(aps[0]))
        return info_str
    ...
    def do_python_eval(self):
        ...
        print('Mean AP@[0.5:0.95] = {:.4f}'.format(np.mean(np.mean(aps, 1))))
        info_str += 'Mean AP@0.5 = {:.4f}\n\n'.format(np.mean(aps[0]))
        info_str += 'Mean AP@[0.5:0.95] = {:.4f}\n\n'.format(np.mean(np.mean(aps, 1)))
        return info_str
